...
async def get_basic_care(plant_name: str):
    ...
    if not care_info:
        raise HTTPException(status_code=404, detail="Informații de îngrijire negăsite")
    return care_info
...
async def get_plant_problems(plant_name: str):
    ...
    if not plant or 'probleme_comune' not in plant:
        raise HTTPException(status_code=404, detail="Informații despre probleme negăsite")
...
async def upload_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Eroare la procesarea imaginii: {str(e)}") from e
